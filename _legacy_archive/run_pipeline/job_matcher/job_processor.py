...
def process_job(job_id: str, cv_text: str, num_runs: int = 5, dump_input: bool = False) -> Dict[str, Any]:
    ...
    try:
     ...
     # Support both beautiful new structure and legacy format
    if "job_content" in job_data:
        # Beautiful new structure! 🌟
        job_content = job_data["job_content"]
        job_title = job_content.get("title", "")
        job_description = job_content.get("description", "")
        print(f"✨ Using beautiful new job structure for job {job_id}")
    else:
        # Legacy format fallback
        web_details = job_data.get("web_details")
        if not web_details:
            print(f"No job content found in job{job_id}.json (neither new format nor legacy)")
            return {"error": "No job content found"}
        
        job_title = web_details.get("position_title", "")
        job_description = prepare_job_description(job_data)
        print(f"📦 Using legacy job structure for job {job_id}")
