...
def determine_domain_gap(llama_eval):
    ...
    # Handle case where llama_eval might be None
    if not llama_eval:
        return 'Unknown'
        
    # Look for domain gap indicators in the evaluation
    domain_assessment = (llama_eval.get('domain_knowledge_assessment') or '').lower()
    no_go_rationale = (llama_eval.get('no_go_rationale') or '').lower()
...
def extract_job_data_for_feedback_system(job_data):
    ...
    # Get match level to help with application narrative
    match_level = llama_eval.get('cv_to_role_match', '')
    
    # Set application narrative based on match level
    application_narrative = llama_eval.get('application_narrative', '')
